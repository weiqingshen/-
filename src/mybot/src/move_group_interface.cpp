...
int main(int argc, char** argv){
  ...
  cylinder_primitive.dimensions[shape_msgs::msg::SolidPrimitive::CYLINDER_HEIGHT] = 0.10;
  ...
  if(success){
    ...
    RCLCPP_INFO(move_group_node->get_logger(),"第二次规划执行成功");
  } else {
    RCLCPP_ERROR(move_group_node->get_logger(),"第二次规划执行失败");
  }

  //移除物体
  planning_scene_interface.removeCollisionObjects({object_to_attach.id});
